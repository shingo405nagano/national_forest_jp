...
## 3. 四角形のメッシュ
指定した面積や辺の長さに基づいて、四角形のメッシュグリッドを生成・操作するための機能を提供します。
### 3-1. インスタンス化
インスタンス化する際に渡した範囲に基づいてメッシュを生成する範囲と単位を決定します。面積や長さをヘクタールやキロメートルなどの単位で使用したい場合は、平面直角座標系などの単位がメートルの座標参照系（CRS）を使用してください。
```python
>>> import geomesh
>>> lon_min, lat_min, lon_max, lat_max = (
....    140.467194155,
...
....    40.990309691,
....)
>>> sq_mesh = geomesh.SquareMesh(lon_min, lat_min, lon_max, lat_max)
```

### 3-2. 面積を指定してメッシュを生成

```python
>>> gdf = square_mesh.generate_squares_from_area(0.001)
>>> gdf = gdf.set_crs(epsg=4326)
>>> print(f"area: {gdf.geometry.iloc[0].area}")
>>> print(gdf)
area: 0.0009999999998941192
      id     xy                                           geometry
0      0    0/0  POLYGON ((140.50008 40.99812, 140.50008 41.029...
1      1    1/0  POLYGON ((140.5317 40.99812, 140.5317 41.02974...
2      2    2/0  POLYGON ((140.56332 40.99812, 140.56332 41.029...
3      3    3/0  POLYGON ((140.59494 40.99812, 140.59494 41.029...
4      4    4/0  POLYGON ((140.62657 40.99812, 140.62657 41.029...
..   ...    ...                                                ...
216  216  12/12  POLYGON ((140.87955 40.61865, 140.87955 40.650...
217  217  13/12  POLYGON ((140.91117 40.61865, 140.91117 40.650...
218  218  14/12  POLYGON ((140.94279 40.61865, 140.94279 40.650...
219  219  15/12  POLYGON ((140.97442 40.61865, 140.97442 40.650...
220  220  16/12  POLYGON ((141.00604 40.61865, 141.00604 40.650...
```

### 3-3. 辺の長さを指定してメッシュを生成
辺の長さは縦横で別々に指定できます。
```python
>>> gdf = square_mesh.generate_squares_from_length(horizontal=0.001, vertical=0.0015)
>>> gdf = gdf.set_crs(epsg=4326)
>>> print(f"length x: {gdf.geometry.iloc[0].area}")
>>> print(gdf)
area: 1.500000000007219e-06
            id       xy                                           geometry
0            0      0/0  POLYGON ((140.46945 41.02824, 140.46945 41.029...
1            1      1/0  POLYGON ((140.47045 41.02824, 140.47045 41.029...
2            2      2/0  POLYGON ((140.47145 41.02824, 140.47145 41.029...
3            3      3/0  POLYGON ((140.47245 41.02824, 140.47245 41.029...
4            4      4/0  POLYGON ((140.47345 41.02824, 140.47345 41.029...
...        ...      ...                                                ...
140963  140963  531/262  POLYGON ((141.00045 40.63524, 141.00045 40.636...
140964  140964  532/262  POLYGON ((141.00145 40.63524, 141.00145 40.636...
140965  140965  533/262  POLYGON ((141.00245 40.63524, 141.00245 40.636...
140966  140966  534/262  POLYGON ((141.00345 40.63524, 141.00345 40.636...
140967  140967  535/262  POLYGON ((141.00445 40.63524, 141.00445 40.636...                             geometry
[140968 rows x 3 columns]
```